'''
由于环境问题，已根据源代码修改，保证模型含义不变，超参数已按照论文指示赋值。
paper: This is the training code for our paper Learning Soft Sparse Shapes for Efficient Time-Series Classification (ICML-25, Spotlight).
...
def Conv1d(ni, nf, kernel_size=None, ks=None, stride=1, padding='same', dilation=1, init='auto', bias_std=0.01,
    ...
    if padding == 'same':
        if kernel_size % 2 == 1:
            ...
            conv = nn.Conv1d(ni, nf, kernel_size, stride=stride, dilation=dilation, **kwargs)
    elif padding == 'causal':
        conv = nn.Conv1d(ni, nf, kernel_size, stride=stride, dilation=dilation, **kwargs)
    elif padding == 'valid':
...
class InceptionModule(nn.Module):
    def __init__(self, ni, nf, ks=40, bottleneck=True):
        ...
        self.bottleneck = Conv1d(ni, nf, 1, bias=False)
        # self.bottleneck = nn.Conv1d(in_channels=ni, out_channels=nf, kernel_size=1, stride=1, bias=False, padding='same')
...
class ShapeEmbedLayer(nn.Module):
    def __init__(self, seq_len, shape_size=8, in_chans=1, embed_dim=128, stride=4):
        super().__init__()
        # 用CNN对时间维度做patch，得到int((seq_len - shape_size) / stride + 1)个patch
        stride = stride
...
class SoftShapeNet_layer(nn.Module):
    ...
    def forward(self, x, end_depth=False, remain_ratio=1.0):
        ...
        if remain_ratio < 1.0:
            ...
            non_topk = torch.gather(x * attn_x_score, dim=1,
                                    index=compl.expand(-1, -1, x.shape[2]))  # [B, N-1-left_tokens, C]
...
class SoftShapeNet(nn.Module):
    def __init__(self, seq_len=args.length, num_channels=args.in_channel, emb_dim=128, num_classes=args.class_num, num_experts=1,
                 stride=2):
        super().__init__()
        ...
        self.moe = MoE_Block(input_size=self.emb_dim, output_size=self.emb_dim, num_experts=num_experts,
        ...
        self.sparse_ratio_d = [x.item() for x in
                               torch.linspace(0, self.sparse_rate, self.depth)]  # stochastic depth decay rule
    ...
    def forward(self, x, num_epoch_i=100, warm_up_epoch=50):
        ...
        x = self.shape_embed(x) # 已经变成(b, pitch, channel)
        x = x + self.pos_embed
        x = self.pos_drop(x)
...
if __name__ == '__main__':
    # model = SoftShapeNet(seq_len=args.seq_len, shape_size=args.shape_size, num_channels=1, emb_dim=args.emb_dim,
    #                      sparse_rate=args.sparse_rate, depth=args.depth, num_experts=args.moe_num_experts,
    #                      num_classes=args.num_class, stride=args.shape_stride)
    model = SoftShapeNet(seq_len=128, shape_size=34, num_channels=3, emb_dim=128,
                         sparse_rate=.8, depth=2, num_experts=5, num_classes=5, stride=4)
